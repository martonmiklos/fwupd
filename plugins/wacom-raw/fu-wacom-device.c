...
fu_wacom_device_set_version_bootloader (FuWacomDevice *self, GError **error)
{
	...
	version = g_strdup_printf ("%x", rsp[RTRN_RSP]);
...
static void
fu_wacom_device_buffer_dump (const gchar *title, const guint8 *buf, gsize sz)
{
			     ...
			     GError **error)
{
	...
	/* Set Feature */
	fu_wacom_device_buffer_dump ("SetFeature", data, datasz);
...
{
	FuWacomDevicePrivate *priv = GET_PRIVATE (self);
	fu_wacom_device_buffer_dump ("GetFeature1", data, datasz);
	...
	fu_wacom_device_buffer_dump ("GetFeature2", data, datasz);
	return TRUE;
}

gboolean
fu_wacom_device_cmd (FuWacomDevice *self,
		     const guint8 *cmd, guint cmdsz,
		     guint8 *rsp, guint rspsz,
		     gulong delay_us, FuWacomDeviceCmdFlags flags,
		     GError **error)
{
	if (!fu_wacom_device_set_feature (self, cmd, cmdsz, error)) {
	...
	if (flags & FU_WACOM_DEVICE_CMD_FLAG_POLL_ON_WAITING &&
		...
		for (guint i = 0; i < FU_WACOM_DEVICE_CMD_RETRIES; i++) {
			...
			if (!fu_wacom_common_check_reply (cmd, rsp, error))
				return FALSE;
			if (rsp[RTRN_RSP] != FU_WACOM_DEVICE_RC_WAITING)
				break;
		}
	}
